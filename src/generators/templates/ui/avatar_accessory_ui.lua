--[[-
  UI module for rendering Accessory Items.
--]]
...
local function get_util()
    return require("Module:Data/AccessoryItem/Util")
end
...
function p.infobox(frame)
    ...
    local item = util.get_accessory_item_by_name_or_id(identifier)
    if not item then return "''Error: Accessory item not found: " .. tostring(identifier) .. ".''" end
...
function p.all(frame)
    ...
    local util = get_util()
    local common = get_common()
    local all_items = util.get_all_accessory_items()
    local L = common.get_i18n(lang)

    local lines = {}
    table.insert(lines, '{| class="wikitable sortable"')
    table.insert(lines, '|-')
    table.insert(lines, '! ID !! Name !! Rarity !! Tribe !! Gender !! Category !! Sort Key !! Detail Type !! Cost')

    for _, item in ipairs(all_items) do
        local name_cell = common.get_name_cell(item, lang, "accessory")
        ...
        local category = ""
        local cost = ""
        if item.accessory_details then
            local d = item.accessory_details
            category = d.category or ""
            cost = d.cost or ""
        end

        table.insert(lines, '|-')
        table.insert(lines, string.format('| %d || %s || %s || %s || %s || %s || %s || %s || %s', 
            item.id, name_cell, rarity_str, getText(L, tribe_label), getText(L, gender_label),
            category, item.sortKey or "", item.detailType or "", cost))
    end
